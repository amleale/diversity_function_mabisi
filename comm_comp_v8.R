# v8 30 june 2023
# Edits from FEMS reviewersß
# richness/Shannon
# post-hoc test of which differ from one another 
# betadisper vegan - test if group centroids or dispersions differ

# please note that legend on line 76 not working properly at moment...
...
#### order by time point
# PROBLEM: names/legend is now incorrect...
reduced %>% 
...
## STACKED BAR PLOT (as suggested by reviewer)
reduced %>% 
 ggplot(aes(x=as.numeric(time), y=abundance, fill=cluster)) + 
  geom_area() +
  scale_fill_manual(labels = new_names$name, values = COLOURS_use)+
...
# pivoted data set is copied from above section for NMDS plots
reduced <- merged %>% 
  filter(barcode != "bc90") %>% 
  filter(barcode != "bc94") %>% 
  filter(barcode != "bc86") %>% 
...
pivoted$richness <- specnumber(pivoted[c(5:10)]) 
pivoted$shannon <- diversity(pivoted[c(5:10)])
...
rich_lm <- lm(richness ~ COMMUNITY*TRANSFER, data = pivoted) 
# Anova(rich_lm, type = 3) 
summary(rich_lm)
emmeans(rich_lm, pairwise ~ COMMUNITY|TRANSFER) 
emmeans(rich_lm, pairwise ~ TRANSFER|COMMUNITY) 
...
shan_lm <- lm(shannon ~ COMMUNITY*TRANSFER, data = pivoted) 
# Anova(shan_lm, type = 3) 
summary(shan_lm)
emmeans(shan_lm, pairwise ~ COMMUNITY|TRANSFER) 
emmeans(shan_lm, pairwise ~ TRANSFER|COMMUNITY) 


##### average relative abundance of each #####
reduced <- merged %>%  # will again use version without missing barcodes added
  filter(barcode != "bc90") %>% 
  filter(barcode != "bc94") %>% 
  filter(barcode != "bc86")  

# to have nicer naming, rename clusters 
new_names2 <- new_names %>% rename(cluster = type)
reduced2 <- merge(reduced, new_names2, by= "cluster") 
reduced2 <- subset(reduced2 , select = -c(cluster, barcode, colour_name, colour, SAMPLE)) #remove not needed rows
reduced2_means <- aggregate(.~COMMUNITY*TRANSFER*name, reduced2, FUN = mean)  #calculate mean 

wide2 <- reduced2_means  %>% #make wide for better visualisation of data
  pivot_wider(names_from = name, values_from = abundance, values_fill = 0)
# write.csv(wide2, "summary_of_means.csv", row.names=FALSE)
...
# mean of total AAB and LAB
wide3 <- subset(wide, select = -c(SAMPLE, barcode))
mean_AAB_LAB <- aggregate(.~COMMUNITY*TRANSFER, wide3, FUN = mean)  #calculate mean 
# write.csv(mean_AAB_LAB, "summary_of_AAB_LAB_means.csv", row.names=FALSE)
...
adonis2(wide[ ,5:18] ~ COMMUNITY*TRANSFER, data = wide[,1:4], method = "bray")  # same output as below "y_permanova" if use full data set
...
wide_subset <- wide %>% filter(TRANSFER == "t17")

dist_dml <- vegan::vegdist(x=as.matrix(wide_subset[,5:18]), 
                           method="bray", binary=FALSE, diag=TRUE, upper=TRUE, na.rm=FALSE)

y_permanova <- vegan::adonis2(dist_dml ~ COMMUNITY,
